...
namespace ratio::core
{
  ...
  {
  public:
    conjunction(scope &scp, context ctx, utils::rational cst, const std::vector<std::unique_ptr<const riddle::ast::statement>> &stmnts);
    conjunction(const conjunction &orig) = delete;
    virtual ~conjunction();

    inline utils::rational get_cost() const noexcept { return cost; } // returns the cost for applying this conjunction..
  ...
  private:
    ...
    const utils::rational cost;                                                // the cost for applying this conjunction..
